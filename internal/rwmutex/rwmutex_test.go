...
var _ = Describe("RwMutex", func() {
	It("Benchmark", Serial, func() {
		...
		Expect(safeLockMedian).To(BeNumerically("<", 150*time.Nanosecond))
		Expect(unsafeLockMedian).To(BeNumerically("<", time.Nanosecond))
		Expect(safeRLockMedian).To(BeNumerically("<", 50*time.Nanosecond))
		Expect(unsafeRLockMedian).To(BeNumerically("<", time.Nanosecond))
	})
})
